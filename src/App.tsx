...
function App() {
  ...
  async function mintNFT() {
    try {
      if (!window.ethereum) throw new Error("Wallet not detected — open in Warpcast or MetaMask");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0A192F] text-white text-center px-4">
      <img
        src="https://i.ibb.co/fGFG1Gz4/ezgif-com-animated-gif-maker.gif"
        alt="Wolfplet"
        className="w-32 h-32 rounded-full mb-6 border-4 border-purple-600 shadow-lg"
      />

      <h1 className="text-3xl font-bold mb-2">Wolfplet NFT</h1>
      <p className="text-gray-300 mb-6 max-w-md">
        Mint your Wolfplet NFT on Base for <b>0.1 USDC</b> and join the pack.
      </p>
      ...
      <p className="mt-6 text-sm text-gray-400 max-w-md">{status}</p>

      <a
        href="https://warpcast.com/~/compose?text=I%20just%20minted%20my%20Wolfplet%20NFT%20on%20Base%20🐺%20join%20the%20pack"
        target="_blank"
        rel="noopener noreferrer"
        className="mt-6 underline text-purple-400 hover:text-purple-300 text-sm"
      >
        Share your mint on Warpcast
      </a>
    </div>
  );
}
